...
function Invoke-DscV3
{
    ...
    if ('Validate' -ne $Operation)
    {
        ...
        $adapterBase = Get-Item (Split-Path -Path $PSScriptRoot -Parent)
        $adapterManifest = $adaptersBase.GetFiles("DscV3.PSCoreAdapter/psDscAdapter.psd1") | Select-Object -First 1

        # load private functions of psDscAdapter stub module
        if ($PSVersionTable.PSVersion.Major -le 5)
        {
            $adapterManifest = $adapterBase.GetFiles("DscV3.PsDesktopAdapter/win_psDscAdapter.psd1") | Select-Object -First 1
        }
        
        $psDscAdapter = Import-Module $adapterManifest.FullName -Force -PassThru
